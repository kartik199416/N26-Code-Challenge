...
public class TransactionControllerTest {
	...
	public void testDeleteTransactions() throws Exception {
		...
		mvc.perform(delete("/transactions").contentType(MediaType.APPLICATION_JSON_UTF8))
				.andExpect(status().is(HttpStatus.NO_CONTENT.value()));

		verify(service, times(1)).deleteTransaction();
	...
	public void testAddTransactionCreated() throws Exception {
		...
		mvc.perform(post("/transactions").contentType(MediaType.APPLICATION_JSON_UTF8).content(toJson(tx)))
	...
	public void testAddTransactionNoContent() throws Exception {
		...
		doThrow(new ExpireTxException()).when(service).addTransaction(any(Transaction.class));
	...
	public void testAddTransactionUnprocessableEntity() throws Exception {
		...
		doThrow(new UnparsableTxException()).when(service).addTransaction(Mockito.any(Transaction.class));
	...
	public void testAddTransactionInvalidJsonFormat() throws Exception {
		...
		mvc.perform(post("/transactions").contentType(MediaType.APPLICATION_JSON_UTF8).content(body))
				.andExpect(status().is(HttpStatus.UNPROCESSABLE_ENTITY.value()));
