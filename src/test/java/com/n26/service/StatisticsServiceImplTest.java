...
public class StatisticsServiceImplTest {
    ...
    public void testConcurrentSumFullSpan() throws InterruptedException {
		...
		//for ex: 10 * 100
		int txCount = Math.toIntExact(StatsCollector.SPAN_MS * StatsCollector.COUNT);
		...
		//read a ms before oldest StatsCollector goes stale
	...
	@Test
    public void testConcurrentSumDiscardOldestStatsCollectorDuringRead() throws InterruptedException {
		...
		//for ex: 10 * 100
		int txCount = Math.toIntExact(StatsCollector.SPAN_MS * StatsCollector.COUNT);
		int acceptedTxCount = txCount - Math.toIntExact(StatsCollector.SPAN_MS);
		...
		//read when the oldest StatsCollector goes stale (array rolls over and discards oldest StatsCollector during read)
		//expect to see 9 * 100 (last StatsCollector is discarded)
	...
	@Test
    public void testConcurrentSumDiscardOldestStatsCollectorWhenOverflow() throws InterruptedException {
		...
		//for ex: 10 * 100
		int txCount = Math.toIntExact(StatsCollector.SPAN_MS * StatsCollector.COUNT);
		int acceptedTxCount = txCount - Math.toIntExact(StatsCollector.SPAN_MS) + 1;
		...
		//array rolls over and new StatsCollector to work on is cleared, has only latest tx inside
